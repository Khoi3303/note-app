...
function loadDraft() {
    try {
        ...
        renderLabelCheckboxes(draft.labelIds || []);
        showSaveStatus('📝 Đã khôi phục ghi chú đang chỉnh sửa');
    } catch (error) {
...
async function loadProfile() {
    try {
        const response = await fetch(`${apiHost}/api/auth/profile`, {
            headers: {
                Authorization: 'Bearer ' + token
            }
        });
        ...
        avatar.className = `profile-avatar ${avatarColor}`;
    } catch (error) {
...
function openProfileModal() {
    ...
    preview.textContent = currentName.charAt(0).toUpperCase();
    preview.className = `profile-avatar large ${selectedAvatarColor}`;
}
...
function selectAvatarColor(color) {
    ...
    const preview = document.getElementById('profile-avatar-preview');
    preview.className = `profile-avatar large ${color}`;
}
...
async function changePassword() {
    ...
    try {
        const response = await fetch(`${apiHost}/api/auth/change-password`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`
