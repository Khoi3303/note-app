...
async function fetchLabels() {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error('Không thể tải labels');
        }
        ...
        console.error(error);
        showToast('Không thể tải labels.');
    }
}
    ...
    globalLabels.forEach(label => {
        ...
        div.innerHTML = `
            ...
            <div class="label-actions">
                <button onclick="event.stopPropagation(); editLabel(${label.id})">✏️</button>
                <button onclick="event.stopPropagation(); deleteLabel(${label.id})">🗑️</button>
            </div>
        `;
...
function renderLabelCheckboxes(selectedIds = []) {
    ...
    container.innerHTML = '';
    globalLabels.forEach(label => {
        const checked = selectedIds.includes(label.id) ? 'checked' : '';
        container.innerHTML += `
...
function getSelectedLabelIds() {
    return Array.from(
        document.querySelectorAll('#label-checkboxes input:checked')
...
async function createLabel() {
    ...
    if (!name) {
        showToast('Tên nhãn không được để trống.');
        return;
    }
    try {
        const response = await fetch(`${apiHost}/api/notes/labels`, {
        ...
        if (response.ok) {
            input.value = '';
            fetchLabels();
            showToast('Đã thêm nhãn.');
        } else {
            const data = await response.json();
            showToast(data.message || 'Tạo nhãn thất bại.');
        }
    } catch {
